...
-   does time spent predict final grades?

-   why did student #54434 get a 65 final grade if they learned the
...
-   the number of cases changes, the number of variables stays the same
...
```{r}
  select(d, student_id, FinalGradeCEMS) |> 
  filter(FinalGradeCEMS > 70) |>
  arrange(desc(FinalGradeCEMS))
```
...
```{r}
m2 <- lm(FinalGradeCEMS ~ TimeSpent - q1 + subject, data = d)
summary(m2)
```
